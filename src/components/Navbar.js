 ...
 const Navbar = (props) => {
    ...
    return (
        <div className="bg-gray-500 w-screen  min-h-[60px] shadow-md  fixed">
          ...
          <div className={`absolute  right-2  gap-1  hidden burger-res cursor-pointer text-white text-2xl top-5 `} onClick={burgerHandler}>
          {display?<GiCrossMark/>:<GiHamburgerMenu />}
            </div>            
        </div>
    )
};
